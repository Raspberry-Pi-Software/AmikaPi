# check for updates
import sys, os, subprocess, requests, time, random, speech_recognition as sr, threading, json
#from pybluez.bluetooth import ble

from . import internet, spotify
...
clientIdentifier = json.load(open('./distribution/stable/client.json'))
...
trainer.train("./distribution/stable/dataset.yml")
...
print(f"> v{open('./distribution/stable/version').readline()}")
...
def check_config_for_spotify_enabled():
    if (os.path.exists('./distribution/stable/setup.cfog')): pass
    else: return False
